...
// onSelect(location, displayName) is called when the user picks an entry.
...
// Renders the full forecast into #results.
...
function renderForecast(data, dateStr, location) {
  ...
  // Aggregate stats over race morning
  ...
  const minFL      = Math.min(...morningFL);
  const maxFL      = Math.max(...morningFL);
